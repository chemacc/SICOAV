...
namespace SICOAV_A.Recursos
{
   ...
   public struct FlightRadarData
   {
      ...
      public string aircraftype;
        ...
        {
            return "[" + Id + "] - " + name + ";" + NVuelo + ";" + ssr + ";" + point.ToString() + ";" + bearing + ";" + altitude + ";" + speed + ";" + aircraftype + ";" + radar + ";" + salida + ";" + llegada;
        }
    }
   ...
   public class Stuff
   {
      ...
      public static void GetFlightRadarData(List<FlightRadarData> ret, RectLatLng bounds)
      {
         ...
         {
                // https://www.flightradar24.com/43.51,-5.87/7
                ...
                var response = GetFlightRadarContentUsingHttp(string.Format(CultureInfo.InvariantCulture, "https://data.flightradar24.com/zones/fcgi/feed.js?bounds={0},{1},{2},{3}&faa=1&mlat=1&flarm=1&adsb=1&gnd=1&air=1&vehicles=1&estimated=1&maxage=900&gliders=1&stats=1&", bounds.Top, bounds.Bottom, bounds.Left, bounds.Right));//44.33, 35.79, -15, 6.09));
                //var response = GetFlightRadarContentUsingHttp(string.Format(CultureInfo.InvariantCulture, "https://data.flightradar24.com/zones/fcgi/feed.js?bounds=-10.756607312117689,-13.287152086983411,-78.19395795214143,-75.78369140625&faa=1&mlat=1&flarm=1&adsb=1&gnd=1&air=1&vehicles=1&estimated=1&maxage=900&gliders=1&stats=1&"));
            ...
            foreach(var it in items)
            {
               if(it.Length > 11 && !it.Contains("full_count") && !it.Contains("stats"))
               {
                  ...
                  if(par.Length >= 9)
                  {
                     ...
                     var lng = par[3];
                     var bearing = par[4];
                     var altitude = (int) (int.Parse(par[5]));
                     var speed = (int) (int.Parse(par[6]));
                     
                     FlightRadarData fd = new FlightRadarData();
                     fd.name = name;
                     fd.bearing = int.Parse(bearing);
                     fd.altitude = altitude.ToString();
                     fd.speed = speed.ToString();
                     ...
                     fd.ssr = par[1];
                     fd.NVuelo = par[10];
                     fd.Tiempo = SICOAV_A.Matematicas.Matematicas.UnixTimeStampToDateTime(double.Parse(par[11]));
                            if (int.Parse(par[5]) != 0)
                            {
                                ret.Add(fd);
                                IB_SGLT_LOG_A.WriteLine(fd.ToString());
                            }
